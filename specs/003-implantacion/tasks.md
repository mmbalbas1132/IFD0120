...
| TI.5 | Pruebas de rendimiento k6 sobre `GET /tareas`, `GET /modulos` | p95 < 2s con las VUs decididas en **CA-09**; informe adjunto al PR | M | TI.2 | `e2e/perf/*.js` | RNF-001, CA-09 |
...
| TI.9 | `docker-compose.prod.yml` + proxy inverso (Nginx/Traefik) con TLS | Stack completo arranca en entorno de prueba, HTTPS válido | M | TI.6 | `docker-compose.prod.yml`, config del proxy | RNF-008, CA-08 |
...
| TI.13 | **[BLOQUEADA por CA-01]** Implementar almacenamiento real de ficheros adjuntos (Entrega/Recurso) | No se despliega a `prod` con datos reales hasta que CA-01 esté resuelta | — | CA-01 | backend + frontend, según decisión | RF-003, RF-012, CA-01 |
...
| TI.15 | Revisión final de trazabilidad RF ↔ UC (tabla de `spec.md` §10) | Cada RF con ≥1 test automatizado que lo cubra, verificado contra la tabla | M | TI.3–TI.6 | checklist, sin fichero nuevo | Todos los RF |

## Cierre del módulo (y del proyecto v1.0)

- [ ] Suite E2E + accesibilidad + rendimiento en verde en CI.
- [ ] `docker-compose.prod.yml` desplegado con éxito en un entorno de prueba.
