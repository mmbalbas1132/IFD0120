...
| TS.26 | **[Desbloqueada 2026-09-23: CA-01 resuelta, ver RNF-014 y §12 de spec.md]** Subida real (`multipart/form-data`, campo `fichero`) y almacenamiento en volumen persistente de los adjuntos de Entrega, Tarea y Recurso DOCUMENTO; descarga por `GET /adjuntos/{id}` con los permisos del elemento; máx. 10 MB (`413`) y solo PDF/ZIP/PNG/JPG/DOCX/ODT por contenido real (`415`) | Pendiente de detallar en `plan.md` de 002 al empezar el módulo (el “cómo”); criterio Done a fijar entonces | — | CA-01 | `{service}/Entrega*.java`, `{service}/Recurso*.java` | RF-003, RF-012 |
